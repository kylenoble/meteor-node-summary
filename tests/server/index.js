Tinytest.add("node-summary", function (test) {
  ...
  SummaryTool.summarize(title, content, function(err, summary) {
      var expectedSummary = "Swayy is a beautiful new dashboard for discovering and curating online content [Invites]\n"
      expectedSummary += "I like it for the simplicity and accuracy of the content curation.\n"
      expectedSummary += "I can share from the service itself, but I generally prefer reading the actual post first – so I end up sharing it from the main link, using Swayy more as a service for discovery.\n"
      expectedSummary += "The idea was born from our day-to-day need to be active on social media, look for the best content to share with our followers, grow them, and measure what content works best."
      test.equal(summary, expectedSummary);

      test.equal((title.length + content.length), 4364);
      test.equal(summary.length, 511);

      var summaryRatio = 100 - (100 * (summary.length / (title.length + content.length)))

      test.equal(summaryRatio, 88.29055912007333);
  });
});
